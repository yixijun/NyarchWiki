...
## 免责声明
该脚本旨在尽可能在任何 Linux 发行版上提供与 Nyarch Linux 相似的体验，而不修改系统文件。因此，请阅读[你错过了什么部分](#what-you-are-missing)。
...
- `yay`，由脚本安装，它们是 AUR 助手，你可以用另一个 AUR 助手安装它。Yay 也被别名为 `nyay`，因为 lol
